...
dataset close all.
output close all.
*** FRS HBAI - change - public v1.sps.
***   Dec 2018.

***   GENERAL NOTES: 
***             WRITTEN BY NICK BAILEY, UNIVERSITY OF GLASGOW. COPYRIGHT CC-BY-SA. 
***             PART OF A SET OF SYNTAX FILES TO PRODUCE THE ANALYSIS FOR:
***                Bailey, Nick (2018) Divisions within 'Generation Rent': poverty and the re-growth of private renting in the UK. 
***                UBDC Working Paper.
***             
...
GET
  FILE='temp/adult.sav'
   /keep sernum benunit person 
            sex age hholder hoh r01 to r14.
dataset name ad.
sort cases by sernum benunit person.
* merge in hh+hb data.
MATCH FILES file=* 
...
GET
  FILE='temp/adult.sav'
   /keep sernum benunit person 
            sex age hholder hrpid combid r01 to r14.
dataset name ad.
sort cases by sernum benunit person.
* merge in hh+hb data.
MATCH FILES file=* 
...
GET
  FILE='temp/adult.sav'
   /keep sernum benunit person 
            sex age80 hholder hrpid combid r01 to r14.
dataset name ad.
sort cases by sernum benunit person.
* merge in hh+hb data.
MATCH FILES file=* 
...
GET
  FILE='temp/hbai.sav'
   /keep sernum benunit 
            adulth depchldh tentyp2 gvtregn newgvtregn 
            low60bhc low60ahc mdscorech mdch lowincmdch lowincmdchsev 
            gs_newhh gs_newbu gs_newad gs_newch gs_newpn  gs_newwa  gs_newpp .
dataset name hb.
* take first case for each hhld - all vars at hhld level so no diffs between benunits.
select if (benunit=1).
...
GET
  FILE='temp/hbai.sav'
   /keep sernum benunit 
            adulth depchldh tentyp2 gvtregn newgvtregn 
            low60bhc low60ahc mdscorech mdch lowincmdch lowincmdchsev mdscorepn mdpn 
            gs_newhh gs_newbu gs_newad gs_newch gs_newpn  gs_newwa  gs_newpp .
dataset name hb.
* take first case for each hhld - all vars at hhld level so no diffs between benunits.
select if (benunit=1).
...
* freq tenure2 tenure3 tenure4.


*** correct tenure3/tenure4 for children.
...
execute.
* make file with one case per hhld and ten of each person.
...
crosstabs yearcode by tenure6 /cells row.
